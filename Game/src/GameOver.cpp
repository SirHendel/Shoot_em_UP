...
void renderGameOverText(SDL_Renderer* renderer, const char* text, float x, float y, TTF_Font* font, SDL_Color color) {
...
void renderGameOverScreen(AppContext& appContext) {
    ...
    renderGameOverText(appContext.renderer, gameOverText.c_str(), 250, 150, appContext.font, redColor);

    SDL_Color whiteColor = {255, 255, 255, 255};
    std::string finalScoreText = getTranslation(GAME_SCORE, selectedLanguage) + ": " + std::to_string(score);
    renderGameOverText(appContext.renderer, finalScoreText.c_str(), 300, 220, appContext.font, whiteColor);

    std::string killsText = getTranslation(GAME_KILLS, selectedLanguage) + ": " + std::to_string(gameStats.enemiesKilled);
    renderGameOverText(appContext.renderer, killsText.c_str(), 300, 250, appContext.font, whiteColor);

    std::string waveText = getTranslation(GAME_WAVE, selectedLanguage) + ": " + std::to_string(currentWave);
    renderGameOverText(appContext.renderer, waveText.c_str(), 300, 280, appContext.font, whiteColor);

    std::string bossText = "Bosses: " + std::to_string(gameStats.bossesDefeated);
    renderGameOverText(appContext.renderer, bossText.c_str(), 300, 310, appContext.font, whiteColor);

    SDL_Color grayColor = {200, 200, 200, 255};
    std::string restartText = "R - " + getTranslation(GAME_RESTART, selectedLanguage);
    renderGameOverText(appContext.renderer, restartText.c_str(), 300, 380, appContext.font, grayColor);

    std::string menuText = "ESC - " + getTranslation(GAME_BACK_TO_MENU, selectedLanguage);
    renderGameOverText(appContext.renderer, menuText.c_str(), 300, 410, appContext.font, grayColor);
